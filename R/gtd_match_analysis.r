...
gtd_event_coverage <- function(g, weight_thres, use_geomatch) {
  ...
  e = subset(g$d, .use)
  e = merge(e, g$to_meta[,c('document_id','terrorism')], by.x='to', by.y='document_id', all.x=T)
  e = e[, list(N_news=length(weight), has_news=T, terrorism_news=sum(terrorism)), by='from']
  ...
  e$terrorism_news[is.na(e$terrorism_news)] = 0
