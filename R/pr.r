...
calculate_gtd_pr <- function(e, min_weight=NA, weight_col='weight', hourdiff_range=c(0,7*24)) {
  ...
  Pm = round(tab[2,2] / sum(tab[2,]),3) * 100
  Rm = round(tab[2,2] / sum(tab[,2]),3) * 100
  F1m = round(2*(Rm * Pm) / (Rm + Pm),1)

  e2 = e[order(-e$real),]
  e2 = e2[!duplicated(e2$to),]
  tab = table(hit=e2$hit_f, real=e2$real_f)
  Pa = round(tab[2,2] / sum(tab[2,]),3) * 100
  Ra = round(tab[2,2] / sum(tab[,2]),3) * 100
  F1a = round(2*(Ra * Pa) / (Ra + Pa),1)
